...
const INPUT_CLASS = "mt-1 block w-full rounded-md border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-gray-100 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500";
...
export default function TradeForm({ trade, strategies, onSave, onCancel }: TradeFormProps) {
  ...
  useEffect(() => {
    if (trade?.compliance && trade.strategy_id === strategyId) {
      ...
      trade.compliance.forEach((c) => { map[c.rule_id ?? c.rule_text] = c.followed; });
      setCompliance(map);
    } else {
      const map: Record<string, boolean> = {};
  ...
  const pnl = entryPrice && exitPrice && quantity
    ? calculatePnl(direction, parseFloat(entryPrice), parseFloat(exitPrice), parseFloat(quantity)) : null;
  const outcome = exitPrice ? (pnl !== null ? determineOutcome(pnl) : 'Open') : 'Open';
  const riskReward = entryPrice && stopLossPrice && quantity
    ? calculateRiskReward(direction, parseFloat(entryPrice), parseFloat(stopLossPrice), exitPrice ? parseFloat(exitPrice) : null, parseFloat(quantity)) : null;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await onSave({
      ...
      compliance: rules.map((r) => ({ rule_id: r.id, rule_text: r.text, followed: compliance[r.id] ?? false })),
    });
    setSaving(false);
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          ...
          <select value={strategyId} onChange={(e) => setStrategyId(e.target.value)} className={INPUT_CLASS}>
        ...
        <div>
          ...
          <input value={symbol} onChange={(e) => setSymbol(e.target.value)} required placeholder="AAPL" className={INPUT_CLASS} />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-300">Direction</label>
          <select value={direction} onChange={(e) => setDirection(e.target.value as 'Long' | 'Short')} className={INPUT_CLASS}>
        ...
        <div>
          ...
          <input type="number" step="any" value={entryPrice} onChange={(e) => setEntryPrice(e.target.value)} required className={INPUT_CLASS} />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-300">Exit Price</label>
          <input type="number" step="any" value={exitPrice} onChange={(e) => setExitPrice(e.target.value)} className={INPUT_CLASS} />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-300">Stop Loss Price</label>
          <input type="number" step="any" value={stopLossPrice} onChange={(e) => setStopLossPrice(e.target.value)} placeholder="Optional" className={INPUT_CLASS} />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-300">Max Loss</label>
          <div className={`mt-1 rounded-md border border-gray-700 bg-gray-800/50 px-3 py-2 text-sm ${exceedsThreshold ? 'text-red-400 border-red-700' : maxLoss !== null ? 'text-yellow-400' : 'text-gray-500'}`}>
            {maxLoss !== null ? formatCurrency(maxLoss) : '--'}
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-300">Quantity</label>
          <input type="number" step="any" value={quantity} onChange={(e) => setQuantity(e.target.value)} required className={INPUT_CLASS} />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-300">P&L</label>
          <div className="mt-1 flex items-center gap-2">
            <div className={`flex-1 rounded-md border border-gray-700 bg-gray-800/50 px-3 py-2 text-sm ${pnl !== null ? (pnl > 0 ? 'text-green-400' : pnl < 0 ? 'text-red-400' : 'text-gray-400') : 'text-gray-500'}`}>
              {pnl !== null ? `$${pnl.toFixed(2)}` : '--'}
            </div>
            {riskReward && <RiskBadge ratio={riskReward.ratio} reward={riskReward.reward} risk={riskReward.risk} exceedsThreshold={exceedsThreshold} />}
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-300">Entry Date</label>
          <input type="datetime-local" value={entryDate} onChange={(e) => setEntryDate(e.target.value)} required className={INPUT_CLASS} />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-300">Exit Date</label>
          <input type="datetime-local" value={exitDate} onChange={(e) => setExitDate(e.target.value)} className={INPUT_CLASS} />
        </div>
      </div>

      <JournalFields thesis={thesis} setThesis={setThesis} lessons={lessons} setLessons={setLessons} tags={tags} setTags={setTags} notes={notes} setNotes={setNotes} />
      <ComplianceChecklist rules={rules} compliance={compliance} onChange={setCompliance} />

      <div className="flex justify-end gap-2 pt-2">
        <button type="button" onClick={onCancel} className="rounded-md px-4 py-2 text-sm text-gray-400 hover:text-white">Cancel</button>
...
function JournalFields({ thesis, setThesis, lessons, setLessons, tags, setTags, notes, setNotes }: {
  thesis: string; setThesis: (v: string) => void;
  lessons: string; setLessons: (v: string) => void;
  ...
  return (
    <>
      <div>
        ...
        <textarea value={thesis} onChange={(e) => setThesis(e.target.value)} rows={2} placeholder="Market thesis for this trade..." className={INPUT_CLASS} />
      </div>
      <div>
        ...
        <textarea value={lessons} onChange={(e) => setLessons(e.target.value)} rows={2} placeholder="What did you learn?" className={INPUT_CLASS} />
      </div>
      <div>
        ...
        <input value={tags} onChange={(e) => setTags(e.target.value)} placeholder="earnings, breakout, FOMO" className={INPUT_CLASS} />
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-300">Notes</label>
        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} className={INPUT_CLASS} />
      </div>
    </>
  );
}

function ComplianceChecklist({ rules, compliance, onChange }: {
  rules: Rule[]; compliance: Record<string, boolean>; onChange: (c: Record<string, boolean>) => void;
}) {
      ...
      <div className="space-y-2 rounded-md border border-gray-700 bg-gray-800/50 p-3">
        {rules.map((rule) => (
          <label key={rule.id} className="flex items-start gap-2 text-sm">
            <input type="checkbox" checked={compliance[rule.id] ?? false} onChange={(e) => onChange({ ...compliance, [rule.id]: e.target.checked })} className="mt-0.5 rounded border-gray-600 bg-gray-700 text-blue-500 focus:ring-blue-500" />
